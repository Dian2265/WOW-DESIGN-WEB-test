...
const SphereImageGrid: React.FC<SphereImageGridProps> = ({
  ...
  const renderImageNode = useCallback((image: ImageData, index: number) => {
    ...
    // VISUAL FIX: Shift Y upwards by a percentage of container size to perfectly center the sphere visual weight
    // The -containerSize * 0.1 shifts everything up by 10%
    const yOffset = -containerSize * 0.1;
    ...
    return (
      <div
        ...
        style={{
          ...
          top: `${containerSize/2 + position.y + yOffset}px`,
  ...
  const renderSpotlightModal = () => {
    ...
    return (
      ...
      >
        <div
          className="bg-wow-black border border-white/20 rounded-xl max-w-md w-full overflow-hidden shadow-2xl"
        ...
        >
          <div className="relative aspect-square">
            ...
            <button
              onClick={() => setSelectedImage(null)}
              className="absolute top-2 right-2 w-8 h-8 bg-black bg-opacity-50 rounded-full text-white flex items-center justify-center hover:bg-opacity-70 transition-all cursor-pointer border border-white/20"
            >
              <X size={16} />
            </button>
          </div>
          ...
          {(selectedImage.title || selectedImage.description) && (
            <div className="p-6 bg-wow-gray/50">
              {selectedImage.title && (
